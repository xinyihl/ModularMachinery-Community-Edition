...
import kport.gugu_utils.tools.CIELab;
...
public abstract class TileSparkManaHatch extends CommonMMTile implements IRestorableTileEntity, ISparkAttachable {
    ...
    public boolean shouldRefresh(World world, BlockPos pos, IBlockState oldState, IBlockState newState) {
        if (oldState.getBlock() != newState.getBlock())
            return true;
        if (oldState.getBlock() != BlocksMM.blockSparkManaProvider || newState.getBlock() != BlocksMM.blockSparkManaProvider)
            return true;
    ...
    public void renderHUD(Minecraft mc, ScaledResolution res) {
        ItemStack hatch = new ItemStack(BlocksMM.blockSparkManaProvider, 1, world.getBlockState(getPos()).getValue(BlockSparkManaHatch.VARIANT).ordinal());
    ...
    public void syncMana(EntityPlayerMP player) {
        ModularMachinery.NET_CHANNEL.sendTo(new PacketMana(getPos(), mana), player);
    }
}
