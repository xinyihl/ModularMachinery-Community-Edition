...
public class TileLargeAspectHatch extends CommonMMTile implements
    ITickable,
    ...
    public void update() {
        ...
        this.existTime++;
        if (isOutput() && !"halt".equals(Config.ASPECT_OUTPUT_HATCH_FULL_ACTION) && !"spill_all".equals(Config.ASPECT_OUTPUT_HATCH_FULL_ACTION)) {
            if (this.essentia.visSize() > Config.ASPECT_OUTPUT_HATCH_MAX_STORAGE) {
                this.spillRandom();
            }
            return;
        }
    ...
    public boolean generate(RequirementAspect.RT inputToken, boolean doOperation) {
        if ("halt".equals(Config.ASPECT_OUTPUT_HATCH_FULL_ACTION)) {
            int generated = Math.min(inputToken.getAmount(), Config.ASPECT_OUTPUT_HATCH_MAX_STORAGE - this.essentia.visSize());
        ...
        if (doOperation) {
            if ("spill_all".equals(Config.ASPECT_OUTPUT_HATCH_FULL_ACTION)) {
                int actuallyGenerated = Math.min(generated, Config.ASPECT_OUTPUT_HATCH_MAX_STORAGE - this.essentia.visSize());
    ...
    public MachineComponent provideComponent() {
        if (isOutput()) {
            //输出，理论上输出没有notifier
            return new GenericMachineCompoment<>((IGeneratable<RequirementAspect.RT>) this, (ComponentType) GuGuCompoments.COMPONENT_ASPECT);
        }
        return new GenericMachineCompoment<>((IConsumable<RequirementAspect.RT>) this, this, (ComponentType) GuGuCompoments.COMPONENT_ASPECT);
    }
